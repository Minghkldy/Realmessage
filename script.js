...
function toggleLeftSidebar() {
    ...
    const labels = sidebar.querySelectorAll('span, .p-6 span');
...
function switchToInbox() {
    ...
    document.querySelectorAll('nav div').forEach(el => el.classList.remove('active-link', 'text-accent-blue'));
    document.getElementById('nav-inbox').classList.add('active-link');
}

function loadBotSettings() {
    showIframe('/bot-config.html', "Bot Config", 'nav-bot');
}

function loadBroadcastSettings() {
    showIframe('/broadcast.html', "Bulk Messaging", 'nav-broadcast');
}

function loadGeneralSettings() {
    showIframe('/general-settings.html', "Settings", null);
}
...
function filterContacts(platform) {
    if (platform === 'all') renderContacts(cachedContacts);
    else renderContacts(cachedContacts.filter(c => c.platform === platform));
}
...
function renderContacts(contacts) {
    ...
    list.innerHTML = contacts.map(c => {
        ...
        return `
            <div onclick='selectContact(${JSON.stringify(c)})' class="flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${isActive ? 'bg-accent-blue/10 border border-accent-blue/20' : 'hover:bg-white/5 border border-transparent'}">
                <div class="relative">
                    <div class="w-10 h-10 rounded-full bg-dark-gray flex items-center justify-center font-bold text-xs border border-border-gray">${c.nickname?.[0] || '?' }</div>
                    <div class="absolute -bottom-1 -right-1 w-4 h-4 bg-pure-black rounded-full flex items-center justify-center">
                        <i class="fab fa-${c.platform} text-[8px] ${c.platform==='facebook'?'text-blue-500':'text-sky-400'}"></i>
                    </div>
                </div>
                <div class="flex-1 min-w-0">
                    <div class="flex justify-between items-center">
                        <span class="text-sm font-bold truncate ${unread > 0 ? 'text-white' : 'text-gray-300'}">${c.nickname || c.chat_id}</span>
                        ${unread > 0 ? `<span class="bg-accent-blue text-[10px] px-1.5 py-0.5 rounded-full font-bold animate-pulse">${unread}</span>` : ''}
                    </div>
                    <div class="text-[10px] text-gray-500 truncate mt-0.5 uppercase tracking-tighter">${c.platform} • Active</div>
                </div>
            </div>
        `;
    }).join('');
...
function renderMessages() {
    ...
    container.innerHTML = allMessages.map(m => {
        ...
        const isImage = m.message_type === 'image' || (m.text && m.text.match(/\.(jpeg|jpg|gif|png)$/) != null);
        
        return `
            <div class="flex ${isBot ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2">
                <div class="max-w-[80%] ${isBot ? 'items-end' : 'items-start'} flex flex-col">
                    <div class="px-4 py-2.5 text-sm shadow-xl ${isBot ? 'chat-bubble-user text-white' : 'chat-bubble-bot text-gray-200 border border-border-gray'}">
                        ${isImage ? `<img src="${m.text}" onclick="showImagePreview('${m.text}')" class="rounded-lg max-w-xs cursor-pointer hover:opacity-90 transition-opacity">` : m.text}
                    </div>
                    <div class="flex items-center gap-2 mt-1 px-1">
                        <span class="text-[9px] text-gray-500 font-bold uppercase tracking-widest">${time}</span>
                        ${isBot ? `<span class="status-indicator text-[9px] font-bold ${m.is_read ? 'text-accent-blue' : 'text-gray-500'}">${m.is_read ? 'SEEN' : 'SENT'}</span>` : ''}
                    </div>
                </div>
            </div>
        `;
